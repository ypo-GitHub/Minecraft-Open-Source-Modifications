...
public class ConfigManager implements Util {
  ...
  public String config = "catware/config/";
  ...
  private static void loadFile(JsonObject input, Feature feature) {
    for (Map.Entry entry : input.entrySet()) {
      ...
      if (feature instanceof FriendManager) {
        ...
        for (Setting setting1 : feature.getSettings()) {
          if (!settingName.equals(setting1.getName()))
            continue; 
          try {
            setValueFromJson(feature, setting1, element);
          } catch (Exception e) {
            e.printStackTrace();
          } 
          settingFound = true;
      ...
      if (!(feature instanceof NoDDoS))
        continue; 
      NoDDoS noDDoS = (NoDDoS)feature;
      Setting setting = feature.register(new Setting(settingName, Boolean.valueOf(true), v -> (((Boolean)noDDoS.showServer.getValue()).booleanValue() && !((Boolean)noDDoS.full.getValue()).booleanValue())));
  ...
  public void loadConfig(String name) {
    ...
    List<File> files = (List<File>)Arrays.<Object>stream((Object[])Objects.requireNonNull((new File("catware")).listFiles())).filter(File::isDirectory).collect(Collectors.toList());
    this.config = files.contains(new File("catware/" + name + "/")) ? ("catware/" + name + "/") : "catware/config/";
  ...
  public void saveCurrentConfig() {
    File currentConfig = new File("catware/currentconfig.txt");
    try {
      if (currentConfig.exists()) {
        FileWriter writer = new FileWriter(currentConfig);
        String tempConfig = this.config.replaceAll("/", "");
        writer.write(tempConfig.replaceAll("catware", ""));
        writer.close();
      } else {
  ...
  public void saveSettings(Feature feature) throws IOException {
    ...
    String featureName;
    Path outputFile;
    if (!Files.exists(outputFile = Paths.get(featureName = this.config + getDirectory(feature) + feature.getName() + ".json", new String[0]), new java.nio.file.LinkOption[0]))
