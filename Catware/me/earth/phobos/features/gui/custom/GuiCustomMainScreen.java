...
public class GuiCustomMainScreen extends GuiScreen {
  ...
  private final String backgroundURL = "https://i.imgur.com/My4Lov3.jpg";
  ...
  public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
    if (isHovered(this.x - Phobos.textManager.getStringWidth("Singleplayer") / 2, this.y + 20, Phobos.textManager.getStringWidth("Singleplayer"), Phobos.textManager.getFontHeight(), mouseX, mouseY)) {
      this.mc.displayGuiScreen((GuiScreen)new GuiWorldSelection(this));
  ...
  public BufferedImage parseBackground(BufferedImage background) {
    ...
    int height;
    for (height = 1080; width < srcWidth || height < srcHeight; ) {
      width *= 2;
      height *= 2;
    } 
  ...
  private static class TextButton extends GuiButton {
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
      if (this.visible) {
        ...
        Phobos.textManager.drawStringWithShadow(this.displayString, this.x - Phobos.textManager.getStringWidth(this.displayString) / 2.0F, this.y, Color.WHITE.getRGB());
        if (this.hovered)
          RenderUtil.drawLine((this.x - 1) - Phobos.textManager.getStringWidth(this.displayString) / 2.0F, (this.y + 2 + Phobos.textManager.getFontHeight()), this.x + Phobos.textManager.getStringWidth(this.displayString) / 2.0F + 1.0F, (this.y + 2 + Phobos.textManager.getFontHeight()), 1.0F, Color.WHITE.getRGB()); 
      } 
    }
