...
public class PhysicsCapes extends Module {
  ...
  public void onPlayerRender(RenderPlayerEvent.Post event) {
    ...
    for (ModelRenderer renderer : this.cape.boxList) {
      waveMap.put(renderer, Float.valueOf((float)Math.sin(fuck / 0.5D) * 4.0F));
      fuck++;
    } 
    double rotate = interpolate((event.getEntityPlayer()).prevRenderYawOffset, (event.getEntityPlayer()).renderYawOffset, event.getPartialRenderTick());
    GlStateManager.translate(0.0F, 0.0F, 0.125F);
    double d0 = (event.getEntityPlayer()).prevChasingPosX + ((event.getEntityPlayer()).chasingPosX - (event.getEntityPlayer()).prevChasingPosX) * event.getPartialRenderTick() - (event.getEntityPlayer()).prevPosX + ((event.getEntityPlayer()).posX - (event.getEntityPlayer()).prevPosX) * event.getPartialRenderTick();
    double d1 = (event.getEntityPlayer()).prevChasingPosY + ((event.getEntityPlayer()).chasingPosY - (event.getEntityPlayer()).prevChasingPosY) * event.getPartialRenderTick() - (event.getEntityPlayer()).prevPosY + ((event.getEntityPlayer()).posY - (event.getEntityPlayer()).prevPosY) * event.getPartialRenderTick();
    double d2 = (event.getEntityPlayer()).prevChasingPosZ + ((event.getEntityPlayer()).chasingPosZ - (event.getEntityPlayer()).prevChasingPosZ) * event.getPartialRenderTick() - (event.getEntityPlayer()).prevPosZ + ((event.getEntityPlayer()).posZ - (event.getEntityPlayer()).prevPosZ) * event.getPartialRenderTick();
    ...
    GL11.glTranslated(0.0D, -((event.getEntityPlayer()).height - (event.getEntityPlayer().isSneaking() ? 0.25D : 0.0D) - 0.38D), 0.0D);
