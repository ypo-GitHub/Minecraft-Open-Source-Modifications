...
public class GuiPeek extends Module {
  ...
  public void postDrawScreen(GuiScreenEvent.DrawScreenEvent.Post event) {
    if (event.getGui() instanceof GuiContainer && ((GuiContainer)event.getGui()).getSlotUnderMouse() != null) {
      ItemStack item = ((Slot)Objects.<Slot>requireNonNull(((GuiContainer)event.getGui()).getSlotUnderMouse())).getStack();
      if (item.getItem() instanceof ItemShulkerBox) {
        this.name = item.getDisplayName();
        this.box = true;
        int X = event.getMouseX() + 8;
        int Y = event.getMouseY() - 82;
        NBTTagList nbttaglist = ((NBTTagCompound)Objects.<NBTTagCompound>requireNonNull(item.hasTagCompound() ? item.getTagCompound() : new NBTTagCompound())).getCompoundTag("BlockEntityTag").getTagList("Items", 10);
        int xMod = 6;
        int yMod = 6;
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("futurex", "shulker.png"));
      ...
      if (mc.player.inventory.getItemStack().getItem() instanceof net.minecraft.item.ItemAir) {
        ...
        if (slotUnderMouse != null && slotUnderMouse.getHasStack()) {
          ...
          if (itemUnderMouse.getItem() instanceof ItemMap) {
            MapData mapdata = ((ItemMap)itemUnderMouse.getItem()).getMapData(itemUnderMouse, (World)mc.world);
            if (mapdata != null) {
