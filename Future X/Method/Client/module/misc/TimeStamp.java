...
public class TimeStamp extends Module {
  ...
  public boolean onPacket(Object packet, Connection.Side side) {
    if (packet instanceof SPacketChat) {
      ...
      if (this.Location.getValString().equalsIgnoreCase("Start"))
        packet2.chatComponent = (ITextComponent)new TextComponentString(TmStamp() + " " + TextFormatting.getValueByName(this.Textcolor.getValString().toLowerCase()) + input + TextFormatting.RESET); 
      if (this.Location.getValString().equalsIgnoreCase("End"))
        packet2.chatComponent = (ITextComponent)new TextComponentString(TextFormatting.getValueByName(this.Textcolor.getValString().toLowerCase()) + input + " " + TextFormatting.RESET + TmStamp() + TextFormatting.RESET); 
    } 
    return true;
  }
  ...
  private String TmStamp() {
    ...
    TextComponentString time = new TextComponentString(TextFormatting.getValueByName(this.BracketColor.getValString().toLowerCase()) + decoLeft + TextFormatting.RESET + TextFormatting.getValueByName(this.Colortype.getValString().toLowerCase()) + date + TextFormatting.RESET + TextFormatting.getValueByName(this.BracketColor.getValString().toLowerCase()) + decoRight + TextFormatting.RESET);
    return time.getText();
  }
}
