...
public class Mixintab extends GuiPlayerTabOverlay {
  ...
  public static final Ordering<NetworkPlayerInfo> ENTRY_ORDERING = GuiPlayerTabOverlay.ENTRY_ORDERING;
  ...
  public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
    ...
    for (j4 = 1; i4 > 20; i4 = (l3 + j4 - 1) / j4)
      j4++; 
    boolean flag = (mc.isIntegratedServerRunning() || ((NetHandlerPlayClient)Objects.<NetHandlerPlayClient>requireNonNull(mc.getConnection())).getNetworkManager().isEncrypted());
    ...
    for (int k4 = 0; k4 < l3; k4++) {
      ...
      if (k4 < list.size()) {
        ...
        if (networkplayerinfo1.getGameType() == GameType.SPECTATOR) {
          mc.fontRenderer.drawStringWithShadow(TextFormatting.ITALIC + s4, j2, k2, -1862270977);
        } else if (FriendManager.isFriend(s4)) {
          mc.fontRenderer.drawStringWithShadow(ExtraTab.FriendObfus.getValBoolean() ? "Friend" : s4, j2, k2, ExtraTab.FriendColor.getcolor());
        } else {
          mc.fontRenderer.drawStringWithShadow(s4, j2, k2, -1);
        } 
  ...
  private int getK1(int width, int k1, int l1, List<String> list1) {
    drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list1.size() * mc.fontRenderer.FONT_HEIGHT, -2147483648);
    for (String s2 : list1) {
      int i2 = mc.fontRenderer.getStringWidth(s2);
      mc.fontRenderer.drawStringWithShadow(s2, (width / 2 - i2 / 2), k1, -1);
  ...
  protected void drawPing(int offset, int x, int yval, NetworkPlayerInfo networkPlayerInfoIn) {
    ...
    if (ExtraTab.ReplaceBar.getValBoolean()) {
      this.zLevel += 100.0F;
      mc.fontRenderer.drawStringWithShadow(String.valueOf(networkPlayerInfoIn.getResponseTime()), (x + offset - 11), yval, -1);
      this.zLevel -= 100.0F;
      return;
    } 
    ...
    this.zLevel += 100.0F;
    drawTexturedModalRect(x + offset - 11, yval, 0, 176 + j * 8, 10, 8);
    this.zLevel -= 100.0F;
  }
      ...
      if (j > 0) {
        ...
        if (f > 3.0F) {
          ...
          if (p_175247_5_ - mc.fontRenderer.getStringWidth(s + "hp") >= p_175247_4_)
            s = s + "hp"; 
          mc.fontRenderer.drawStringWithShadow(s, ((p_175247_5_ + p_175247_4_) / 2 - mc.fontRenderer.getStringWidth(s) / 2), p_175247_2_, i1);
        } 
      } 
    } else {
      String s1 = TextFormatting.YELLOW + "" + i;
      mc.fontRenderer.drawStringWithShadow(s1, (p_175247_5_ - mc.fontRenderer.getStringWidth(s1)), p_175247_2_, 16777215);
    } 
  }
