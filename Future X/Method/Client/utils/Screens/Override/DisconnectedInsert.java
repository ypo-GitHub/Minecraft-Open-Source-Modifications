...
public class DisconnectedInsert extends Screen {
  ...
  public void GuiScreenEventPost(GuiScreenEvent.ActionPerformedEvent.Post event) {
    if (event.getGui() instanceof GuiDisconnected) {
      ...
      if ((event.getButton()).id == 999) {
        ...
        mc.displayGuiScreen((GuiScreen)new GuiConnecting((GuiScreen)new GuiMainMenu(), mc, lastserver.serverIP, serveraddress.getPort()));
      } 
      if ((event.getButton()).id == 998) {
        Autolog = !Autolog;
        double Starttime = (System.currentTimeMillis() + 5000L);
        (new Thread(() -> {
              try {
  ...
  public void GuiScreenEventInit(GuiScreenEvent.InitGuiEvent.Post event) {
    ...
    if (event.getGui() instanceof GuiDisconnected) {
      event.getButtonList().add(new GuiButton(999, (event.getGui()).width / 2 - 100, Math.min((event.getGui()).height / 2 + 10 + (event.getGui()).fontRenderer.FONT_HEIGHT, (event.getGui()).height - 30) + 20, 200, 20, "Relog"));
      event.getButtonList().add(new GuiButton(998, (event.getGui()).width / 2 - 100, Math.min((event.getGui()).height / 2 + 10 + (event.getGui()).fontRenderer.FONT_HEIGHT, (event.getGui()).height - 30) + 40, 200, 20, "Auto 5s Relog"));
      event.getButtonList().add(new GuiButton(997, (event.getGui()).width / 2 - 100, Math.min((event.getGui()).height / 2 + 10 + (event.getGui()).fontRenderer.FONT_HEIGHT, (event.getGui()).height - 30) + 60, 200, 20, "Ping"));
      if (Autolog) {
        Autolog = false;
        ...
        MinecraftForge.EVENT_BUS.post((Event)new GuiScreenEvent.ActionPerformedEvent.Post(event.getGui(), button, (event.getGui()).buttonList));
      } 
    } 
  }
}
