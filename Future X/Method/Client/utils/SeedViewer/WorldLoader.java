...
public class WorldLoader {
  ...
  public static void setup() {
    ...
    if (Wrapper.mc.player.dimension == -1) {
      ChunkGenerator = (IChunkGenerator)new ChunkGeneratorHell(fakeworld, fakeworld.getWorldInfo().isMapFeaturesEnabled(), seed);
    } else {
  ...
  public static Chunk CreateChunk(int x, int z, int dis) {
    ...
    if (!fakeworld.isChunkGeneratedAt(x, z)) {
      ...
      Testchunk = fakeworld.getChunk(x, z);
    } 
    (fakeworld.getChunkProvider()).loadedChunks.put(ChunkPos.asLong(x, z), Testchunk);
    Testchunk.onLoad();
    populate((IChunkProvider)fakeworld.getChunkProvider(), ChunkGenerator, x, z);
    return Testchunk;
  }
