...
public class EventsHandler {
      ...
      (new Thread(() -> {
            try {
              while (Wrapper.mc.currentScreen != Main.ClickGui) {
                Wrapper.mc.displayGuiScreen((GuiScreen)Main.ClickGui);
                Thread.sleep(25L);
  ...
  public void renderNamePlate(RenderLivingEvent.Specials.Pre e) {
    try {
      if (NameTags.toggled && 
        e.getEntity() instanceof net.minecraft.entity.player.EntityPlayer)
        e.setCanceled(true); 
    } catch (RuntimeException ex) {
  ...
  public void PlayerLoggedInEvent(PlayerEvent.PlayerLoggedInEvent event) {
    try {
      if (event.player != null) {
        ArrayList<IRecipe> recipes = Lists.newArrayList((Iterable)CraftingManager.REGISTRY);
